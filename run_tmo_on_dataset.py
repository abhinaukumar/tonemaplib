...
def run_tmo_on_video(args, video_path, standard, out_video_path, device=None):
    ...
    print(f'Calling TMO {type(tmo).__name__} on {video.file_path}')
    tmo(video, out_video_path)
...
def main():
    ...
    parser.add_argument('--out_format', help='Format of output video (Default: compressed).', type=str, required=False, default='encoded')
    ...
    TMOClass = get_tmoclass(args.tmo)
    tmo_args, tmo_kwargs = load_args(args.tmo_args_file)
    tmo_kwargs['out_format'] = args.out_format
    tmo = TMOClass(*tmo_args, **tmo_kwargs)
    ...
    elif args.out_format == 'encoded':
        out_video_list = [os.path.join(out_dir, os.path.splitext(os.path.basename(video_path))[0] + '.mp4') for video_path in video_list]
    else:
        raise ValueError('Invalid output format {}'.format(args.out_format))
